 ...
 * </p>
 */
public class JsonBodyAvroSer implements EventSerializer, Configurable {

    private static final Logger logger =
            LoggerFactory.getLogger(JsonBodyAvroSer.class);
    ...
    private JsonBodyAvroSer(OutputStream out) {
        this.out = out;
    }
    ...
    public void write(Event event) throws IOException {
        ...
        //convert binary json string to avro format
        String jsonStr = new String(event.getBody());
        Map<String,Object> maps = (Map) JSON.parse(jsonStr);
        DatumWriter<GenericRecord> mGr = new SpecificDatumWriter<GenericRecord>(schema);
        GenericRecord gr = new GenericData.Record(schema);
        for(Map.Entry<String,Object> entity: maps.entrySet()){
            gr.put(entity.getKey(),entity.getValue());
        }
        //ByteArrayOutputStream baos = new ByteArrayOutputStream();
        byte[] value = null;
        try{
            BinaryEncoder binaryEncoder = EncoderFactory.get().binaryEncoder(baos, null);
            mGr.write(gr, binaryEncoder);
            binaryEncoder.flush();
        ...
        dataFileWriter.appendEncoded(ByteBuffer.wrap(value));
    }
    ...
    public static class Builder implements EventSerializer.Builder {
        ...
        public EventSerializer build(Context context, OutputStream out) {
            JsonBodyAvroSer writer = new JsonBodyAvroSer(out);
